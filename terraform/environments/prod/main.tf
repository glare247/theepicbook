...
terraform {
  ...
  # Docs: https://developer.hashicorp.com/terraform/language/backend/gcs
  backend "gcs" {
...
# ── Shared WIF pool: grant dev WIF principal access to prod SA ───────
...
resource "google_service_account_iam_member" "prod_sa_wif_from_dev_pool" {
...
resource "google_service_account_iam_member" "prod_sa_token_creator_from_dev_pool" {
  service_account_id = module.serviceaccounts.github_actions_sa_name
